...
def plot_heading_results():
    ...
    # 绘制均值曲线
    ax.plot(x, mean_curve, color=colors[0], label="Heading Task", linewidth=2)
    
    # 添加标准差阴影
    ax.fill_between(x, mean_curve - std_curve, mean_curve + std_curve, color=colors[0], alpha=0.2)
    
    # 输出最终性能
    print(f"Heading任务最终值: {mean_curve[-1]:.2f} ± {std_curve[-1]:.2f}")
    
    # 设置图表标题和标签
    ax.set_title("Heading Task Training Performance", fontsize=14)
    ...
    # 调整布局并保存
    plt.tight_layout()
    output_dir = "/home/dqy/NeuralPlanex/AeroPlanex_v/AeroPlanax/plot/plot_result/heading/new/"
    plt.savefig(f"{output_dir}heading_task.png", bbox_inches="tight")
    plt.savefig(f"{output_dir}heading_task.pdf", bbox_inches="tight")
    print(f"\n图表已保存为 {output_dir}heading_task.png 和 {output_dir}heading_task.pdf")
    plt.show()
