                ...
                def _update_epoch(update_state, unused):
                    def _update_minbatch(train_states: Tuple[TrainState,], batch_info):
                        ...
                        def _loss_fn(network_params, init_hstate, traj_batch: Transition, gae, targets):
                            ...
                            entropy = ((pi[0].entropy() + pi[1].entropy() + pi[2].entropy()) * traj_batch.valid_action).sum() / (traj_batch.valid_action.sum() + 1e-8)
...
@desperated
'''
def make_train_union(config, env : LogWrapper, networks : Tuple[nn.Module, None], train_mode: bool=True, save_epochs: int=1):
    '''
    networks: 以ActorCritic的一个网络形式输入
    save_epoch: 当目前epoch%save_epoch时保存

    NOTE: ["NUM_ALLIES"] in config.keys()
    '''
    (network, _) = networks
    ...
    def train(rng, train_states : Tuple[TrainState,TrainState], start_epoch : int = 0):
        ...
        reset_rng = jax.random.split(_rng, config["NUM_ENVS"])
        obsv, env_state = jax.vmap(env.reset, in_axes=(0))(reset_rng)
        network_init_hstate = ScannedRNN.initialize_carry(config["NUM_ACTORS"] * config["NUM_ENVS"], config["GRU_HIDDEN_DIM"])
